...
SET(WX_CONFIG_ARGS_LIBS "${WX_CONFIG_ARGS} --libs std")

IF(WXWIDGETS_USE_MEDIA)
	SET(WX_CONFIG_ARGS_LIBS "${WX_CONFIG_ARGS_LIBS},media")
...
ELSE( WIN32_STYLE_FIND )
    ...
    IF(APPLE)
    #replace the space with underscore, otherwise separating the arguments fails
    STRING(REPLACE "-framework " "-framework_" WX_CONFIGARGS ${WX_CONFIGARGS})
    ENDIF(APPLE)
    ...
    FOREACH(WXARG ${WX_CONFIGARGS})
      ...
      ELSE(WXARG MATCHES "^-L")
        ...
        ELSE(WXARG MATCHES "^-l")
	IF(WXARG MATCHES "^-framework")
		SET(WXWIDGETS_LIBRARIES "${WXWIDGETS_LIBRARIES} ${WXARG}")
	ELSE(WXARG MATCHES "^-framework")
          IF(WXARG MATCHES "^-")
            SET(WXWIDGETS_EXE_LINKER_FLAGS "${WXWIDGETS_EXE_LINKER_FLAGS} ${WXARG}")
          ELSE(WXARG MATCHES "^-")
            SET(WXWIDGETS_LIBRARIES "${WXWIDGETS_LIBRARIES} ${WXARG}")
          ENDIF(WXARG MATCHES "^-")
	ENDIF(WXARG MATCHES "^-framework")  
	...
	IF(APPLE)
		SET(WXWIDGETS_LIBRARIES_REAL "")
		#replace underscore with space again
		FOREACH(WXARG ${WXWIDGETS_LIBRARIES})
			STRING(REPLACE "-framework_" "-framework " WXARG ${WXARG})
			SET(WXWIDGETS_LIBRARIES_REAL "${WXWIDGETS_LIBRARIES_REAL};${WXARG}")
		ENDFOREACH(WXARG ${WXWIDGETS_LIBRARIES_REAL})
		SET(WXWIDGETS_LIBRARIES ${WXWIDGETS_LIBRARIES_REAL})
	ENDIF(APPLE)
